//! Synthetic load generation (Task 5.2).
//!
//! [`SyntheticGenerator`] produces WS-shaped events — `ManifoldWsEvent::NewBets`, each carrying a
//! single synthetic `Bet` — structurally identical to what the real Manifold WebSocket feeds the
//! collector pipeline. [`synthetic_stream`] wraps a generator in a rate-paced async `Stream` so
//! the harness can drive the real `run_ingest_loop`/`run_writer_loop` with instrumentation on and
